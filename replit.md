...
- **Per-mission color themes** (`src/lib/missionThemes.ts`, `src/index.css` `.theme-mission-*`, `src/pages/Settings.tsx`): each of the 42 missions can be assigned a color theme that recolors its Mission Detail, Set Test, and Study Mode pages. Six themes ship by default — **Sunrise** (orange→pink, the brand), **Twilight** (violet/pink/fuchsia, the legacy palette), **Ocean** (sky/blue/cyan), **Forest** (emerald/teal), **Ember** (amber/red), and **Royal** (indigo/purple). Each theme defines a hero gradient (used in MissionDetail's hero card), a 3-stop set-accent palette (stripes, pills, icons, buttons for Set 1/2/3 cards), a 3-stop study-accent palette (group-select cards in StudyMode), and a CSS class `.theme-mission-<id>` that remaps Tailwind's `--color-violet-*` and `--color-fuchsia-*` palette CSS variables to the theme's two color families. The remap technique cascades through entire page subtrees, so every existing `bg-violet-500`, `text-fuchsia-700`, `border-violet-200`, etc. inside a themed wrapper picks up the new color automatically with zero per-className changes. The default for Mission 1 is `brand`; all others default to `violet` (a no-op class that keeps the original palette). User selections are stored in `localStorage` under `gre_mission_themes` as `Record<missionDay, themeId>` and exposed through `AppContext` via `missionThemes`, `getMissionThemeId(day)`, `getMissionTheme(day)`, `setMissionTheme(day, id)`, and `resetMissionTheme(day)`. The Settings page has a **Mission Color Themes** section with a search box and a per-mission row that expands to a 6-tile theme picker showing each theme's gradient preview, label, description, and a "Reset to default" link when the user has overridden a mission's theme.
