...
- **Mission 1 brand color theme** (`src/index.css` `.theme-mission-1`): a CSS-variable scope override that remaps Tailwind's `--color-violet-*` and `--color-fuchsia-*` palette to the brand orange/pink palette inside any subtree wrapped with the class. This recolors entire pages (Mission Detail, Set Test, Study Mode) for Mission 1 to match the orange→pink hero header gradient without touching individual classNames. The wrapper class is added conditionally on each page root: `MissionDetail` (when `missionDay === 1`), `SetTest` (both in-progress and results screens, when `missionDay === 1`), and `StudyMode` (group-select, rich-study, and standard-study views, when `selectedDay === 1`). On `MissionDetail`, the per-set `SET_ACCENTS` array also has a `BRAND_SET_ACCENTS` variant (orange / orange→pink gradient / pink) used for Mission 1 across Set 1 / Set 2 / Set 3 cards. On `StudyMode`'s group-select view, a parallel `brandAccents` array replaces the rainbow set palette for Mission 1. Other missions remain unchanged.
